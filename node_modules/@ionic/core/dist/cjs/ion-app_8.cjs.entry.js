...
const frameworkDelegate = require('./framework-delegate-09d9e72b.js');
const index$1 = require('./index-25b54f72.js');
  ...
  componentDidLoad() {
    {
      rIC(async () => {
        ...
        if (!ionicGlobal.config.getBoolean('_testing')) {
          Promise.resolve().then(function () { return require('./tap-click-6a86d251.js'); }).then(module => module.startTapClick(ionicGlobal.config));
        }
        if (ionicGlobal.config.getBoolean('statusTap', isHybrid)) {
          Promise.resolve().then(function () { return require('./status-tap-7184dd19.js'); }).then(module => module.startStatusTap());
        }
        if (ionicGlobal.config.getBoolean('inputShims', needInputShims())) {
          Promise.resolve().then(function () { return require('./input-shims-054b1cda.js'); }).then(module => module.startInputShims(ionicGlobal.config));
        }
...
const RouterOutlet = class {
  ...
  async connectedCallback() {
    ...
    this.gesture = (await Promise.resolve().then(function () { return require('./swipe-back-65d297e0.js'); })).createSwipeBackGesture(this.el, () => !this.gestureOrAnimationInProgress && !!this.swipeHandler && this.swipeHandler.canStart(), () => onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
      if (this.ani) {
        this.ani.onFinish(() => {
          this.gestureOrAnimationInProgress = false;
          if (this.swipeHandler) {
